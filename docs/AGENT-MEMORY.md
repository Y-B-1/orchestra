...
- **claude-runtime** · `.claude/agents/`, `.claude/skills/orchestrator/`, `docs/orchestra/claude-models.md` · 2026-09-02 · Claude Code is the source runtime; `.cursor/` is generated from `.claude/` by `sync-agent-config.py`. Matrix: Fable 5.1 low (judgement + repair, builder-max included) / Sonnet 5 medium (execution) / Sonnet 5 low (scout). No Opus row anywhere.
...
- **skill-lock** · `.cursor/skills/*/SKILL.md` `disable-model-invocation: true` · 2026-09-02 · Cursor-only, emitted by `sync-agent-config.py` for the phase-playbook mirrors it still writes as skills. The Claude `orchestrator` skill must NOT carry the flag — it is how the main session loads it by description.
