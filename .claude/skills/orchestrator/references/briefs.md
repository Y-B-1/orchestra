...
LEDGER/MEMORY-CANDIDATES/OPEN/CONTEXT-GAP trailer. DONE or BLOCKED.
```
...
4. SWEEP [L3]: expired RESEARCH.md — check every `docs/orchestra/RESEARCH-*.md` against
   its own `expires:` header line and delete any past it; list what you found by name in
   your report; [DEBUG-] lines, harvested throwaway branches, dead .orchestra/ state.
5. CONTEXT-GAP CLUSTER: read every CONTEXT-GAP trailer line from this batch's ledger
   excerpt, cluster by cause. A cluster with 2+ hits gets ONE proposal: a rule edit, a
   rule deletion, or a `docs/orchestra/failures.md` entry — never act on a single remark.
Report: Worktrees / Memory / Adherence / Sweep / Context-gap clusters. You propose; the orchestrator disposes.
```
