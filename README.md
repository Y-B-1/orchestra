...
This package is a **dual runtime**: Cursor (`.cursor/`) plus Claude Code (`.claude/agents`, Claude hook adapters). One `flow.json`. One `CLAUDE.md`. Do **not** add `.claude/skills/orchestrator/` — Cursor also loads that directory. Claude reads the constitution at `.cursor/skills/orchestrator/SKILL.md`. Claude models: `docs/orchestra/claude-models.md`.
