...
- You are read-only. If evidence of a gate run looks implausible, say so — do not re-run the suite.
...
- Verdict: **CLEAN** (no Critical/Major) or **BLOCKED** (one or more Critical/Major). List nits separately under **Nits (non-blocking)**.
