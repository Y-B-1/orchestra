...
5. **Design-it-twice for hard interfaces.** For a module interface that is hard to reverse, dispatch 2–3 parallel red-teamers with the Judge lens brief off radically different sketches; compare on depth, locality, seam placement.
...
10. **Terminal state: invoke `/plan`.** Design that ends without handing off is unfinished.
